...
Check that the output of sipeed_ikernel is as expected when run as a
commandline script.
...
def test_launcher_doing_nothing():
    ...
    result = env.run("sipeed_ikernel", expect_error=True)
    assert result.returncode != 0
    # Help is useless, really
    result = env.run("sipeed_ikernel", "--help")
    assert "usage: sipeed_ikernel" in result.stdout


def test_launcher_version():
    """Launcher not manager."""
    # version goes to stderr in PY2, stdout in PY3
    result = env.run("sipeed_ikernel", "-V", expect_stderr=True)
...
def test_manage_doing_nothing():
    ...
    result = env.run("sipeed_ikernel", "manage")
    assert "usage" in result.stdout
    result = env.run("sipeed_ikernel", "manage", "--help")
    assert "usage" in result.stdout


def test_manage_basics():
    result = env.run("sipeed_ikernel", "manage", "-V", expect_stderr=True)
    assert "Remote Jupyter kernel manager" in result.stdout + result.stderr
    # Show will not error, even with no kernels
    result = env.run("sipeed_ikernel", "manage", "--show")
...
def test_minimum_args():
    ...
    result = env.run("sipeed_ikernel", "manage", "--delete", "rik_local_name")
...
def test_unicode():
    ...
    assert u"\u2602" in result.stdout
    result = env.run(
        "sipeed_ikernel", "manage", "--delete", "rik_local_name_launch_pe___via_th__"
    )
    assert "Removed kernel" in result.stdout
